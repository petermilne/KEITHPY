...
filename = "./results/acq424_lin_"+timestamp+".csv"
...
voltages = np.arange(-10,11,1)   # Specify voltages to loop through
volt_array = []

for i in range (0, len(voltages)):
    v_str = str(voltages[i])
    print "V = "+v_str+"V"
    volt_array.append([])   # Add another sublist to master list
    keith_func.set_AO_all(voltages[i],nchan)   # Set AO on current channel
    time.sleep(3)
    
    channel_volts_array = keith_func.getKVolts_all(nchan)
    
    for channel in range (1,nchan+1):
        
        volt_array[i].append(channel_volts_array[channel-1])
        
        # Record Requested DAC Voltage
        if channel == nchan: volt_array[i].append(voltages[i]); print volt_array[i]

    csv_write.writerow(volt_array[i])  # Write to CSV, format as below
...
#plt.plot(code_array,np.arange(-10,11,1),'r-o')
#plt.show()
